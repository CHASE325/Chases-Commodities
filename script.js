...
function toggleMode(mode){
  ...
  if(mode === 'blackjack'){
    ...
    if(modeBtn) modeBtn.textContent = 'Back to Slots';
    ...
    if(paytable) paytable.classList.remove('hidden');
    if(modeBtn) modeBtn.textContent = 'Switch to Blackjack';
...
function evaluate(matrix){
  ...
  if(surprise > 0){
    msg = msg ? `${msg} + Surprise bonus ${surprise}!` : `Surprise! You earned ${surprise} extra chips!`;
  }

  const delta = totalWin + surprise - bet;
  if(delta > 0){
...
function onChest(e){
  ...
  if(chosen === winner){
    const reward = 3;
    coins += reward; updateUI(); setMessage(`Nice! You found ${reward} chips.`);
  } else {
...
function endWashGame(){
  ...
  coins += reward;
  updateUI();
  setMessage(`Minigame over — you washed ${washed} plates and earned ${reward} chips.`);
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  setupMiniGame();
  const modeBtn = $('#modeBtn');
  if(modeBtn){ modeBtn.addEventListener('click', ()=>{ showMode(gameMode === 'slots' ? 'blackjack' : 'slots'); }); }
  const dealBtn = $('#dealBtn');
  const hitBtn = $('#hitBtn');
  const standBtn = $('#standBtn');
